...
- Authenticated `AppShell` owns the persistent video-background system behind all interface content. Kling 2.5 (`fervo-gold-smoke-kling-2-5.mp4`) remains the global and Public/Distance Feed atmosphere. The Friends Feed requests the calmer Seedance 2.5 atmosphere through the same shared system, using the web-compatible H.264 derivative (`fervo-gold-smoke-seedance-2-5-h264.mp4`). The original HEVC file (`fervo-gold-smoke-seedance-2-5.mp4`) is retained untouched as the master asset. Both served backgrounds are muted, looping, inline videos with `object-fit: cover`; the existing dark centre treatment remains a stationary overlay so Feed content stays visually dominant.
- Feed background changes use a 700ms opacity crossfade. The requested video must be playing before it becomes visible, the previous video remains visible during loading, and the inactive video is paused after the transition. Seedance is loaded lazily on first use and both media elements remain stable across ordinary application navigation. The former CSS translation/scale animation of the PNG is removed rather than running beneath either video.
