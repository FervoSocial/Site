...
Implemented only visual polish: removed the old main Feed heading, added a fluid gold atmosphere with reduced-motion support, short interaction transitions, larger existing avatar/ring treatments and accessible Like/Save and header tooltips. The current production candidate replaces the experimental moving-PNG technique with one shared application-shell video system: Kling 2.5 remains global and serves Public/Distance, while the Friends Feed selects the calmer Seedance 2.5 H.264 web derivative through a restrained crossfade; its original HEVC file remains the untouched master asset. The unchanged PNG remains the poster/static fallback, and all foreground content remains stationary. No prominent motion control is placed in the header. See [Design System](DESIGN_SYSTEM.md) for exact presentation changes. Image replacement is partially implemented: no approved demo profile photos were available, so safe fallbacks remain rather than invented identities.
